...
export class PermissionComponent implements OnInit {
  ...
  trackId = (item: IPermission): number => this.permissionService.getPermissionIdentifier(item);
  ...
  search(query: string): void {
    ...
    if (query && predicate && PermissionComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(predicate)) {
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      const { predicate } = this.sortState();
      if (predicate && PermissionComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(predicate)) {
        this.sortState.set({});
      }
    }
  }
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.permissions.set(dataFromBody);
  }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.permissionService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
